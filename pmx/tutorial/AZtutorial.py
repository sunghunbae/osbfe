...
class AZtutorial:
    ...
    def __init__(self, **kwargs):
        ...
        self.replicas = 3        
        self.simTypes = ['em','eq_nvt', 'eq', 'transitions']
    ...
    def _prepare_single_tpr( self, simpath, toppath, state, simType, empath=None, nvtpath=None, frameNum=0 ):

        mdpPrefix = ''
        if simType=='em':
            mdpPrefix = 'em'
        elif simType=='eq_nvt': # added
            mdpPrefix = 'eq_nvt'
        elif simType=='eq':
            mdpPrefix = 'eq'
            ...
            for state in self.states:
                for r in range(1,self.replicas+1):
                    ...
                    if bLig==True:
                        ...
                        nvtpath = self._get_specific_path(edge=edge,wp=wp,state=state,r=r,sim='eq_nvt')
                        empath = self._get_specific_path(edge=edge,wp=wp,state=state,r=r,sim='em')
                        toppath = ligTopPath
                        self._prepare_single_tpr( simpath, toppath, state, simType, empath, nvtpath )
            ...
            for state in self.states:
                for r in range(1,self.replicas+1):            
                    ...
                    if bLig==True:
                        ...
                        xtc = '{0}/traj_comp.xtc'.format(simpath)
                        dhdl = '{0}/dhdl.xvg'.format(simpath)
                        cpo = '{0}/state.cpt'.format(simpath)
                        self._run_mdrun(tpr=tpr,trr=trr,xtc=xtc,cpo=cpo,ener=ener,confout=confout,mdlog=mdlog,dhdl=dhdl,bVerbose=bVerbose)
                        self._clean_backup_files( simpath )
